...
exports.eventService = {
    ...
    getById(id) {
        const stmt = database_1.database.instance.prepare('SELECT * FROM events WHERE id = ?');
    ...
    remove(id) {
        const stmt = database_1.database.instance.prepare('DELETE FROM events WHERE id = ?');
        stmt.run(id);
    },
    ...
    getRegistrationById(id) {
        const stmt = database_1.database.instance.prepare('SELECT * FROM event_registrations WHERE id = ?');
        const row = stmt.get(id);
    ...
    removeRegistration(id) {
        const stmt = database_1.database.instance.prepare('DELETE FROM event_registrations WHERE id = ?');
        stmt.run(id);
    },
    ...
    getMatchById(id) {
        const stmt = database_1.database.instance.prepare('SELECT * FROM matches WHERE id = ?');
        const row = stmt.get(id);
    ...
    removeMatch(id) {
        const stmt = database_1.database.instance.prepare('DELETE FROM matches WHERE id = ?');
        stmt.run(id);
    },
    standings(eventId) {
