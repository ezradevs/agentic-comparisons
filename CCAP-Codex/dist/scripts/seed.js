...
const seedUsers = async () => {
    const row = database_1.database.instance.prepare('SELECT COUNT(*) as count FROM users').get();
    if (row.count > 0)
        return;
    await userService_1.userService.create({
...
const seedMembers = () => {
    const row = database_1.database.instance.prepare('SELECT COUNT(*) as count FROM members').get();
    if (row.count > 0)
        return;
    const members = [
        { first_name: 'Sophia', last_name: 'Lopez', rating: 1820, email: 'sophia.lopez@example.com', status: 'active', join_date: '2023-01-14' },
        { first_name: 'Henry', last_name: 'Kim', rating: 1685, email: 'henry.kim@example.com', status: 'active', join_date: '2023-03-22' },
        { first_name: 'Maya', last_name: 'Singh', rating: 1450, email: 'maya.singh@example.com', status: 'active', join_date: '2022-11-05' },
        { first_name: 'Elena', last_name: 'Petrov', rating: 1910, email: 'elena.petrov@example.com', status: 'active', join_date: '2021-09-17' },
        { first_name: 'Noah', last_name: 'Reed', rating: 1320, email: 'noah.reed@example.com', status: 'guest', join_date: '2024-01-02' },
        { first_name: 'Lucas', last_name: 'Chen', rating: 2045, email: 'lucas.chen@example.com', status: 'active', join_date: '2020-06-11' },
        { first_name: 'Amelia', last_name: 'Garcia', rating: 1580, email: 'amelia.garcia@example.com', status: 'active', join_date: '2023-07-19' },
        { first_name: 'Ethan', last_name: 'Patel', rating: 1710, email: 'ethan.patel@example.com', status: 'inactive', join_date: '2022-02-12' }
    ];
    members.forEach((member) => memberService_1.memberService.create(member));
    console.log('Seeded members.');
};
const seedEvents = () => {
    const row = database_1.database.instance.prepare('SELECT COUNT(*) as count FROM events').get();
    if (row.count > 0)
        return;
    const registrationIndex = new Map();
    const upcomingTournament = eventService_1.eventService.create({
    ...
    const trainingCamp = eventService_1.eventService.create({
    ...
    members.slice(0, 6).forEach((member, idx) => {
        const registration = eventService_1.eventService.createRegistration({
        ...
        if (idx < 4) {
            eventService_1.eventService.updateRegistration(registration.id, { score: idx % 2 === 0 ? 1 : 0.5 });
        }
    });
    ...
    if (campRegistrations.length >= 4) {
        ...
        const match1 = eventService_1.eventService.createMatch({
            event_id: trainingCamp.id,
            round: 1,
            board: 1,
        ...
        if (white1RegId) {
            eventService_1.eventService.updateRegistration(white1RegId, { score: 1 });
        }
        const black1RegId = registrationIndex.get(`${match1.event_id}:${match1.black_member_id}`);
        if (black1RegId) {
            eventService_1.eventService.updateRegistration(black1RegId, { score: 0 });
        }
        eventService_1.eventService.createMatch({
            event_id: trainingCamp.id,
            round: 1,
            board: 2,
        ...
        if (white2RegId) {
            eventService_1.eventService.updateRegistration(white2RegId, { score: 0.5 });
        }
        const black2RegId = registrationIndex.get(`${trainingCamp.id}:${black2.member.id}`);
        if (black2RegId) {
            eventService_1.eventService.updateRegistration(black2RegId, { score: 0.5 });
        }
    }
    console.log('Seeded events, registrations, and matches.');
};
const seedAnnouncements = () => {
    const row = database_1.database.instance.prepare('SELECT COUNT(*) as count FROM announcements').get();
    if (row.count > 0)
        return;
    announcementService_1.announcementService.create({
