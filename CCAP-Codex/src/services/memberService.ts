...
const serializeMember = (row: any): Member => ({
  id: row.id,
...
export const memberService = {
  list(options: ListMembersOptions = {}): Member[] {
    ...
    const stmt = database.instance.prepare(`SELECT * FROM members ${where} ORDER BY last_name, first_name`);
    const rows = stmt.all(params);
    return rows.map(serializeMember);
  },

  getById(id: string): Member | null {
    const stmt = database.instance.prepare('SELECT * FROM members WHERE id = ?');
  ...
  remove(id: string) {
    const stmt = database.instance.prepare('DELETE FROM members WHERE id = ?');
    stmt.run(id);
  },

  countByStatus(): { status: MemberStatus; total: number }[] {
    const stmt = database.instance.prepare(`
    ...
    `);
    const rows = stmt.all();
    return rows.map((row: any) => ({ status: row.status as MemberStatus, total: Number(row.total) }));
  }
};
